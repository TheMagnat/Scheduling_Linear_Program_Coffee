...
doTheBreak = LpVariable.dicts("doTheBreak", (range(n)), cat=LpBinary)
j = formationDayIndex
for i in range(n):
    model += doTheBreak[i] == lpSum(start[i][j][k] for k in range(rushEndIndex, o))
...
for i in range(n):
    for j in range(m):
        ...
        if j == formationDayIndex and inFormation[i]:
            ...
            model += lpSum(x[i][j][k] for k in range(o)) <= (maxHalfHoursPerDay[i] - formationStopHourIndex + 1 - doTheBreak[i])
...
if atLeastOneIndex:
    for j in range(m):
        ...
        for k in range(rushStartIndex, rushEndIndex):
            if forceAtLeastOne:
                model += lpSum(x[i][weekDay][k] for i in atLeastOneIndex) >= 1
            else:
                model += atLeastOne[weekDay][k] >= 1 - lpSum(x[i][weekDay][k] for i in atLeastOneIndex)
...
started = LpVariable.dicts("started", (range(n)), cat=LpBinary)
finished = LpVariable.dicts("finished", (range(n)), cat=LpBinary)
for i in range(n):
    model += started[i] * m >= lpSum(startEarly[i][j] for j in range(m) if j != formationDayIndex)
    model += finished[i] * m >= lpSum(startLate[i][j] for j in range(m) if j != formationDayIndex)
...
if forceSameStartDuringWeek:
    for i in range(n):
        model += started[i] + finished[i] - 1 <= 0
...
if activateRestrictions:
    ...
    for i, pplEndRestrictions in enumerate(endBefore):
        print(pplEndRestrictions)
        for endRestriction in pplEndRestrictions:
            print(endRestriction)
            j = endRestriction[0]
